...
router.post("/admin/products/create", upload.single("image"), async (req, res) => {
  try {
    ...
    // Convert size to an array of numbers (comma-separated)
    let sizeArray = [];
    if (size) {
      sizeArray = size.split(",").map(Number); // Convert to an array of numbers
    }

    // Create a new product with the categoryId from the form
    const newProduct = new Product({
      ...
      availability,
      description,
      categoryId, // Associate the product with the selected category
    });

    if (req.file) {
      newProduct.imageUrls = `/uploads/${req.file.filename}`; // Handle the image upload
    }

    await newProduct.save(); // Save the product to the database
    res.redirect("/admin/products"); // Redirect after successful product creation
  } catch (err) {
...
// Route to render the form for creating a product with categories
router.get("/admin/products/create", async (req, res) => {
  try {
    const categories = await Category.find(); // Fetch all categories
    res.render('admin/createProduct', { categories }); // Pass categories to the view
  } catch (err) {
    console.error("Error fetching categories:", err);
    res.status(500).send("Error fetching categories");
  }
